...
import { useProject, PROJECTS_ENABLED } from '../lib/projectContext'
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: 'var(--paper)' }}>
      ...
      >
        ...
        {/* Hidden while projects are off (single flat workspace).          */}
        {PROJECTS_ENABLED && (
        ...
        {PROJECTS_ENABLED ? (
          ...
          <>
            {/* Single flat workspace — nav at top, flat routes, no shelf. */}
            <nav className="pt-3 pb-2 space-y-0.5">
              <PrimaryNavItem to="/dashboard" icon={Sparkles}      label="Home" />
              <PrimaryNavItem to="/vera"      icon={MessagesSquare} label="VERA" />
              <PrimaryNavItem to="/review"    icon={CheckSquare}   label="Review" badge={pendingCount} />
              <PrimaryNavItem to="/knowledge" icon={BookOpen}      label="Knowledge" />
              <PrimaryNavItem to="/brain"     icon={Brain}         label="Brain" />
              <PrimaryNavItem to="/measure"   icon={BarChart3}     label="Measure" />
            </nav>
      ...
      {/* Center + right rail. The chat dock is GONE — VERA is now a tab     */}
      {/* (/p/:slug/vera) with the full thread + right-rail draft artifact.  */}
      {/* One conversational surface, reached deliberately. The right rail   */}
      {/* renders only when a page provides content via useRightRail().      */}
