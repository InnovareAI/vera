...
export default function Review() {
  ...
  const filtered = posts.filter(p => {
    if (activeTab === 'Pending Review') return isPending(p.status)
  ...
  const tabCounts = STATUS_TABS.reduce((acc, tab) => {
    acc[tab] = posts.filter(p => {
      if (tab === 'Pending Review') return isPending(p.status)
      if (tab === 'Approved') return p.status === 'Approved' || p.status === 'approved'
