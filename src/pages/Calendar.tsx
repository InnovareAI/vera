...
export default function Calendar() {
  ...
  function postsForDay(day: number): Post[] {
    return posts.filter(p => {
      if (!p.fields['Scheduled Date']) return false
      const d = new Date(p.fields['Scheduled Date'])
            ...
            {cells.map((day, i) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[56px] rounded-xl p-1.5 cursor-pointer transition-all ${
                    isSelected
                ...
                >
                  <div className={`text-xs font-medium mb-1 w-5 h-5 flex items-center justify-center rounded-full ${
                    isToday ? 'text-white' : isSelected ? 'text-violet-700' : 'text-gray-700'
                  }`}>
                    {day}
                  </div>
                    ...
                    {dayPosts.slice(0, 3).map(post => (
                      <div
                        ...
                        className={`h-1.5 w-1.5 rounded-full ${
                          PLATFORM_COLORS[post.fields.Platform || ''] || 'bg-gray-400'
                        }`}
                      />
                    ))}
                    {dayPosts.length > 3 && (
                      <span className="text-[9px] text-gray-400">+{dayPosts.length - 3}</span>
                    )}
                  </div>
                </div>
              )
        ...
        {selectedDay ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-4 sticky top-0">
            ...
            {selectedPosts.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-8">No posts scheduled this day</p>
            ) : (
                  ...
                  <div key={post.id} className="bg-gray-50 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      {post.fields.Platform && (
                        <div className={`w-2 h-2 rounded-full ${PLATFORM_COLORS[post.fields.Platform] || 'bg-gray-400'}`} />
                      )}
                      <span className="text-xs font-medium text-gray-700">
                        {post.fields.Platform || 'Unknown'}
                      </span>
                    ...
                    <p className="text-xs font-medium text-gray-900">{post.fields.Title || 'Untitled'}</p>
                    <p className="text-xs text-gray-500 mt-1 line-clamp-2">{post.fields.Body}</p>
                  </div>
                ))}
              </div>
            )}
